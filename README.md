...
UnoPim 3.0 ships with Laravel 12-style auto-discovery, so the service provider is registered automatically through `composer.json` (`extra.laravel.providers`). You only need to require the package and run the installer.
...
3. **Register the service provider**
    ...
    > UnoPim 3.0 follows the Laravel 12 bootstrap layout — providers live in `bootstrap/providers.php`, **not** in `config/app.php`.
